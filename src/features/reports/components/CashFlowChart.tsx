  ...
  return (
    <Card className="p-4 bg-white border border-slate-100 shadow-sm flex flex-col justify-between h-full min-h-[300px]">
      ...
      <div className="flex items-center justify-between mb-1 border-b border-slate-100 pb-2">
        <div>
          <h3 className="text-xs font-black text-slate-800 uppercase tracking-wider">Fluxo de Caixa Mensal</h3>
          <p className="text-[8px] text-slate-400 font-bold uppercase tracking-wider mt-0.5">Entradas, Saídas e Saldo</p>
        </div>
        <div className="flex items-center gap-1 bg-slate-50 px-2 py-1 rounded-lg border border-slate-100 text-[8px] font-black uppercase text-slate-500 cursor-pointer">
          Mensal
        ...
        {!hasData ? (
          ...
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={data} margin={{ top: 10, right: 5, left: -25, bottom: 0 }}>
              ...
              <Bar dataKey="Saídas" fill="#f43f5e" radius={[3, 3, 0, 0]} maxBarSize={16} />
              <Line type="monotone" dataKey="Saldo" stroke="#0f172a" strokeWidth={2} dot={{ r: 2 }} />
            </ComposedChart>
          </ResponsiveContainer>
        )}
      </div>
