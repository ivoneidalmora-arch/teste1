...
export const importParserService = {
   ...
   * Reutiliza toda a lógica de parsing de colunas/valores/datas já existente.
   */
  async _parseXlsLegacy(file: File): Promise<ImportedTransaction[]> {
    try {
      const allRows = await readXlsFile(file);
      if (!allRows.length) throw new Error('A planilha não possui dados para importar.');

      // Encontrar linha de cabeçalho (mesma lógica do fluxo ExcelJS)
      let headerRowIndex = 0;
      let headers: string[] = [];

      for (let i = 0; i < Math.min(20, allRows.length); i++) {
        const cellValues = (allRows[i] ?? []).map((c: any) => String(c ?? ''));
        const matches = cellValues.filter((cell) => {
          const str = String(cell)
            ...
            .replace(/[\u0300-\u036f]/g, '')
            .trim();
          return ['data', 'placa', 'valor', 'cliente', 'servico', 'categoria', 'amount', 'date', 'veiculo'].some(
            (h) => str.includes(h)
          );
        });
        if (matches.length >= 2) {
      ...
      // Construir objetos no mesmo formato que o ExcelJS produziria
      ...
      // Reutiliza exatamente o mesmo mapeamento de colunas do fluxo xlsx
      return rows.map((row, index) => {
        const rawValorBrutoStr = String(getValueByAliases(row, COLUMN_ALIASES.valorBruto) || '');
        const rawDateStr = String(getValueByAliases(row, COLUMN_ALIASES.data) || '');
        const rawClientStr = String(getValueByAliases(row, COLUMN_ALIASES.cliente) || '');
        ...
        let dateObj = parseBrazilianDate(rawDateStr);
        const rawDateVal = getValueByAliases(row, COLUMN_ALIASES.data);
        if (rawDateVal instanceof Date) dateObj = rawDateVal;
