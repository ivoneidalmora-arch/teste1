...
export const ingestionService = {
  ...
  async parseExcel(file: File): Promise<{ data: IngestionResult, rawResponse: string, logs: string[] }> {
    ...
    addLog(`Iniciando leitura de arquivo Excel/CSV: ${file.name}`);
    ...
    try {
      ...
      if (extension === 'xls') {
        addLog('Formato .xls detectado. Usando SheetJS para compatibilidade.');
        const xlsRows = await readXlsFile(file);
        return this._processRawRows(xlsRows, addLog);
      }
      ...
      throw new Error(`Falha ao ler o arquivo Excel/CSV: ${err.message}`);
    }
  },

  /**
   * Extrai linhas brutas (array de arrays) e as processa com mapToStandard.
   * Usado pelo fluxo .xls (SheetJS) para evitar duplicar a lógica de header.
   */
  async _processRawRows(
    rows: any[][],
    addLog: (msg: string) => void
  ): Promise<{ data: IngestionResult; rawResponse: string; logs: string[] }> {
    const logs: string[] = [];

    if (rows.length === 0) throw new Error('Nenhuma linha encontrada no arquivo.');

    rows.slice(0, 5).forEach((row, idx) => {
      addLog(`DEBUG XLS: Linha ${idx + 1} = ${JSON.stringify(row)}`);
    });
    ...
    for (let i = 0; i < Math.min(rows.length, 50); i++) {
      ...
      if (!row || !Array.isArray(row)) continue;
      const rowStr = row
        .map((c) => stripHtml(String(c)).toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, ''))
        .join('|');
      const matches = headerKeywords.filter((kw) => rowStr.includes(kw)).length;
      if (matches > maxMatches) {
        maxMatches = matches;
    ...
    addLog(`Colunas identificadas: ${headers.filter((h: any) => h).join(', ')}`);
    ...
    return {
      data: normalized,
      rawResponse: 'Dados processados localmente via SheetJS (.xls)',
      logs,
    };
  },
  ...
  mapToStandard(row: any): Transaction {
    ...
    // Fallback: busca a ÚLTIMA coluna que seja um número
    ...
    return {
      ...
      valorLiquido: (serviceName === 'Vistoria Cautelar') 
        ? valorNumerico 
