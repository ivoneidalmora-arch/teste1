          ...
          <h3 className="text-sm font-black text-[#0F172A] flex items-center gap-2">
            ...
            {status.connected ? (
              <span className={cn(
                ...
                {status.status === 'active' ? 'Google Conectado' : 'Reconectar'}
              </span>
            ) : (
              <span className="px-1.5 py-0.5 rounded bg-slate-50 text-slate-400 border border-slate-100 text-[8px] font-black uppercase tracking-widest">
                Modo Offline
              </span>
            )}
          </h3>
            ...
            >
              ...
              <div className="flex flex-col gap-0.5 overflow-hidden">
                {sortedEvents.slice(0, 3).map(event => (
                  <div 
                    key={event.id}
                    className="h-1 w-full rounded-full px-1"
                    style={{ backgroundColor: event.color || '#2563eb' }}
                    title={event.title}
                  />
                ))}
                {dayEvents.length > 3 && (
                  <span className="text-[7px] font-black text-slate-300 text-center">+{dayEvents.length - 3}</span>
                )}
              </div>
            </div>
          );
      ...
      <div className="p-3 bg-slate-50/50 border-t border-slate-50 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[9px] font-bold text-slate-500 uppercase tracking-tight">
            {status.connected ? `Sincronizado: ${status.email}` : 'Integração Pendente'}
          </span>
        </div>
        ...
        {!status.connected || status.status === 'reconnect_required' ? (
          ...
          >
            {status.status === 'reconnect_required' ? 'Reconectar' : 'Ativar Google'} <ExternalLink className="w-2.5 h-2.5" />
          </button>
        ) : (
          <button 
            onClick={loadEvents}
            className="flex items-center gap-1 text-[9px] font-black text-slate-400 hover:text-slate-600 transition-colors uppercase tracking-widest"
          >
            Sincronizar <RefreshCw className="w-2.5 h-2.5" />
          </button>
        )}
      </div>
