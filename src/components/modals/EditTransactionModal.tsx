...
export function EditTransactionModal({ isOpen, onClose, onSuccess, transaction }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const success = await storageService.updateTransaction(transaction.id, updatedData);
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex gap-3 pt-2">
          ...
          >
            {loading ? <span className="animate-spin w-5 h-5 border-2 border-white/30 border-t-white rounded-full"></span> : <><Save className="w-5 h-5" /> Salvar Alterações</>}
          </button>
        </div>
      </form>
    </BaseModal>
